from src.data.twitter.TwitterRawData import iget_json_data_entry
import unittest

class TwitterRawDataTests(unittest.TestCase):
    def test_iget_json_data_entry(self):
        '''
        Tests the iget_json_data_entry method to make sure it is actually importing raw twitter data json objects

        Takes about 20 seconds to run
        '''
        twitter_json_raw_entries = [json_object for json_object in iget_json_data_entry()]

        # test to make sure actually extracted something from json files
